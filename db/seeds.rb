...
users = [
User.create(password: "password", customer_id: customer1.id),
User.create(password: "password", display_name: "j3", customer_id: customer2.id),
User.create(password: "password", display_name: "SkrilleX", customer_id: customer3.id),
User.create(password: "password", display_name: "Norway", customer_id: customer4.id),
User.create(password: "password", display_name: "Whitney Houston", customer_id: customer6.id),
User.create(password: "password", display_name: "Funky Monkey", customer_id: customer7.id)
]

users[2].platform_admin = true
users[2].save
...
approved_stores = [
  ...
  Store.new(name: "World of Woolcraft", path: "world-of-woolcraft", description: "Knitting's no joke"),
  Store.new(name: "Frank's Monsterporium", path: "monsterporium", description: "Monster?  Maybe.  Cuddly?  Definitely.")
]
...
approved_stores.each do |s|
  ...
  user_role.store_id = s.id
  user_role.save

  10.times do
    c = Category.create(title: Faker::Lorem.words[0], store_id: s.id)
    10.times do
      p = Product.create(title: Faker::Address.street_name.split[0],
